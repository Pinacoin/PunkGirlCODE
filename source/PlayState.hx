...
class PlayState extends MusicBeatState
{
	...
	var lowBop:FlxSprite;
	var highBop:FlxSprite;
	var santa:FlxSprite;
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'take-back' | 'rocket' | 'hope-seeker':
			{
					curStage = 'rockconcert';
				
					defaultCamZoom = 0.9;

					var bg:FlxSprite = new FlxSprite(-410, -200).loadGraphic(Paths.image('rockconcert/skyBG'));
                    bg.antialiasing = true;
                    bg.scrollFactor.set(0.9, 0.9);
                    bg.active = false;
                    add(bg);
  
                    var stageFront:FlxSprite = new FlxSprite(-410, -275).loadGraphic(Paths.image('rockconcert/stage'));
					...
					lowBop = new FlxSprite(-350, 90);
					lowBop.frames = Paths.getSparrowAtlas('rockconcert/lowBop');
					lowBop.animation.addByPrefix('bop', 'Low Bop', 24, false);
					lowBop.antialiasing = true;
						lowBop.scrollFactor.set(0.9, 0.9);
						lowBop.setGraphicSize(Std.int(lowBop.width * 1));
					lowBop.updateHitbox();
					add(lowBop);

					highBop = new FlxSprite(-350, 675);
					highBop.frames = Paths.getSparrowAtlas('rockconcert/highBop');
					highBop.animation.addByPrefix('bop', 'High Bop', 24, false);
					highBop.antialiasing = true;
						highBop.scrollFactor.set(0.9, 0.9);
						highBop.setGraphicSize(Std.int(highBop.width * 1));
					highBop.updateHitbox();

					var stageFront:FlxSprite = new FlxSprite(-410, -170).loadGraphic(Paths.image('rockconcert/lights'));
                    stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
                    stageFront.updateHitbox();
                    stageFront.antialiasing = true;
                    stageFront.scrollFactor.set(0.9, 0.9);
			...
			if (isStoryMode)
				{
					...
					if (storyPlaylist.length <= 0)
					{
						...
						if (curSong == "Hope-Seeker"){
							FlxG.switchState(new EndingState(accuracy >= 70));
							
							
						if (accuracy >= 70){
							
							FlxG.sound.playMusic(Paths.music("ThwokOne"),1,false);
						}else{
	...
	*/
	...
	override function beatHit()
	{
		...
		{ 
			case 'rockconcert':
				lowBop.animation.play('bop', true);
				highBop.animation.play('bop', true);
